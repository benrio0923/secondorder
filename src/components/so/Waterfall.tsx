...
export function Waterfall({ p, m }: { p: PriceResult; m: Market }) {
  ...
  const color = (k: string) =>
    k === 'china' ? 'bg-amber/70' : k === 'tax' ? 'bg-halt/45' : k === 'channel' ? 'bg-chan' : 'bg-line2'

  return (
    <div>
      <div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 font-mono text-[10px] text-ink2">
        <span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-sm bg-amber/70" />你的成本</span>
        <span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-sm bg-halt/45" />目的国税费</span>
        <span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-sm bg-chan" />渠道加价</span>
      </div>
        ...
        {rows.map((r, i) => {
          ...
          return (
            <div key={i} className={`grid grid-cols-[minmax(96px,132px)_1fr_auto] items-center gap-2.5 ${isMark ? 'py-1' : ''}`}>
              <div className={`truncate text-[11.5px] ${isMark ? 'font-medium text-ink' : 'text-ink2'}`} title={r.label}>
                {r.label}
                {r.src && <SourceTag source={r.src} asOf={r.asOf} />}
              </div>
              <div className="relative h-[18px] overflow-hidden rounded-sm bg-paper2">
                {isMark ? (
                  <div className="absolute inset-y-0 left-0 border-r-2 border-line2" style={{ width: `${w}%` }} />
                ) : (
                  <>
                    <div className="absolute inset-y-0 left-0 bg-paper2" style={{ width: `${start}%` }} />
                    <div
              ...
              <div className="w-[92px] text-right">
                {isMark ? (
                  <span className="font-mono text-[12.5px] font-semibold tabular-nums text-ink">{fmt(r.cum)}</span>
                ) : (
                  <span className="font-mono text-[11.5px] tabular-nums text-ink2">＋{fmt(r.amount)}</span>
                )}
              </div>
            </div>
          )
      ...
      <div className="mt-4 space-y-1.5 border-t border-line pt-3 text-[11.5px] leading-relaxed text-ink2">
        {rows
          .filter((r) => r.changing)
          .map((r, i) => (
        ...
        <p>
          终端零售约 <Num className="text-ink">{fmt(p.retailLocal)}</Num> {m.currency}
          ，是内销开票价的 <Num className="text-ink">{p.multiple}</Num> 倍；落地税负占 到岸价 的{' '}
          <Num className="text-ink">{Math.round((p.taxTotal / p.cif) * 100)}%</Num>。
        </p>
      </div>

      <div className="mt-4 rounded border border-line bg-paper2 p-3.5">
        <div className="mb-2.5 font-mono text-[10px] uppercase tracking-[0.14em] text-ink2">
              ...
              return (
                <div key={i} className="grid grid-cols-[1fr_auto] items-center gap-3">
                  <div className="min-w-0">
                    <div className={`mb-1 truncate text-[11.5px] ${mine ? 'font-medium text-amber' : 'text-ink2'}`}>{b.name}</div>
                    <div className="h-[7px] overflow-hidden rounded-sm bg-paper2">
                      <div
                        className={`h-full rounded-sm ${mine ? 'bg-amber' : 'bg-line2'}`}
                        style={{ width: `${Math.max(2, (b.priceLocal / bmax) * 100)}%`, transition: 'width .6s ease' }}
                      />
                    </div>
                  </div>
                  <span className={`w-[104px] shrink-0 text-right font-mono text-[11px] tabular-nums ${mine ? 'text-amber' : 'text-ink3'}`}>
