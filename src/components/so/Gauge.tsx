...
export function RiskDial({ risk }: { risk: RiskResult }) {
  ...
  return (
    <div className="flex items-center gap-5">
      <svg viewBox="0 0 130 76" className="w-[150px] shrink-0" role="img" aria-label={`套利風險 ${pct} 分`}>
        ...
        <text x="65" y="72" textAnchor="middle" fontSize="8.5" fill="#8B928B" letterSpacing="1.6">RISK / 100</text>
      </svg>
      <div className="min-w-0">
        ...
        <div className="mt-2 font-mono text-[10.5px] text-stone/70">
          已判定 {risk.answered}／{SIGNALS.length} 項訊號
        </div>
      </div>
    </div>
  )
            ...
            <div className="mb-2 flex items-center justify-between gap-2">
              ...
              <div className="flex shrink-0 gap-1">
                {([true, null, false] as const).map((opt) => {
                  const on = v === opt
                  const label = opt === true ? '正' : opt === false ? '負' : '？'
                  const cls = on
                    ? opt === true
                  ...
                  return (
                    <button
                      ...
                      title={opt === true ? s.good : opt === false ? s.bad : '資訊不足'}
                    >
                      {label}
                    </button>
                  )
