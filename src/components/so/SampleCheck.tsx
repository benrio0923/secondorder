...
export function SampleCheck({ abv, asked }: { abv: number; asked: boolean }) {
  ...
  const tone =
    r.band === 'free'
      ? { border: 'border-l-emerald-500/60', bg: 'bg-emerald-950/15', text: 'text-emerald-300' }
      : r.band === 'class3'
        ? { border: 'border-l-rose-500/60', bg: 'bg-rose-950/18', text: 'text-rose-300' }
        : { border: 'border-l-rose-500', bg: 'bg-rose-950/25', text: 'text-rose-300' }
  ...
  return (
    <div className={`rounded border-l-2 ${tone.border} ${tone.bg} px-4 py-3.5`}>
      ...
      <div className="mb-1.5 text-[13.5px] font-medium text-bone">{r.title}</div>
      <p className="mb-2.5 text-[12px] leading-relaxed text-stone">{r.detail}</p>

      {r.band !== 'free' && (
        <>
          <div className="mb-1.5 font-mono text-[10px] uppercase tracking-[0.12em] text-stone">你得先备齐</div>
          <ul className="mb-2.5 space-y-1">
            {r.needs.map((n) => (
              <li key={n} className="flex gap-2 text-[12px] leading-snug text-bone">
          ...
          <p className="text-[12px] leading-relaxed text-amber">
            ...
            <span className="text-bone">先问清楚对方能不能收，再答应寄</span>
            ——不然货会卡在仓库，而对方以为你在拖。
          </p>
        </>
      )}
      ...
      <div className="mt-3 border-t border-white/8 pt-3">
        ...
        <div className="relative h-[38px]">
          <div className="absolute inset-x-0 top-[15px] h-[3px] rounded bg-white/8" />
          <div
        ...
        <div className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-[10.5px] text-stone/70">
          {ABV_THRESHOLDS.map((t) => (
            <span key={t.abv}>
              <b className="text-stone">{t.abv}°</b> {t.label}
            </span>
          ))}
        </div>
      </div>
      <p className="mt-2.5 text-[10.5px] leading-snug text-stone/55">依据：{r.source}。实际以承运商与目的国规定为准。</p>
    </div>
  )
}
